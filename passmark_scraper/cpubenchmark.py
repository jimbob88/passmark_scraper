...
class single_cpu:
    def __init__(
        self, id=None, name=None, mark=None, rank=None, value=None, price=None
    ):
        self._url = None
        self._id = id
        self.id = id
...
class mega_cpu:
    ...
    def __dict__(self):
        return {
            ...
            "cpu_price": self._cpu_price,
            "cpu_mark": self._cpu_mark,
            "cpu_value": self._cpu_value,
            "thread_mark": self._thread_mark,
            "thread_value": self._thread_value,
            "tdp": self._tdp,
...
class cpubenchmark_scraper_single:
    ...
    def refresh_soup(self, source=None, features="html.parser"):
        ...
        try:
            ...
            logger.critical(
                "BS4 failed to create a BeautifulSoup instance, error: %s" % e
            )
            raise e
    ...
    def refresh_table(self, soup=None):
        ...
        if _table is None:
            logger.critical(
                "BS4 was unable to find any table on this website by the id 'cputable'"
            )
            raise Exception(
                "BS4 failed to find any table on this website by the id 'cputable'"
            )
        else:
    ...
    def refresh_headers(self, table=None):
        ...
        try:
            ...
            logger.critical(
                "BS4 was unable to find any headers on this table, error: %s" % e
            )
            raise e
        logger.info(
            "BS4 successfully discovered headers on this table: %s" % _header_cols_txt
        )
        return _header_cols
    ...
    def refresh_raw_rows(self, table=None):
        ...
        try:
            ...
            logger.critical(
                "BS4 was unable to find any rows on this table, error: %s" % e
            )
            raise e
    ...
    def refresh_cpus(self, raw_rows=None, headers=None):
        ...
        for idx, row in enumerate(raw_rows):
            ...
            if len(_cols) < 5:
                ...
                if _id is None:
                    logger.warning(
                        "%s: unable to find CPU id, the url returned will most likely be wrong."
                    )
                _cpus.append(
                    single_cpu(
                        id=_id,
        ...
        for idx, row in enumerate(raw_rows):
            ...
            if len(_cols) < 11:
                ...
                _temp_cpu = dict(
                    ...
                    test_date=_cols[8].text,
                    socket=_cols[9].text,
                    category=_cols[10].text,
                )
            ...
            if len(_cols2) < 5:
                ...
                _temp_cpu.update(
                    {
                        "clock_speed": _cols2[0].text.replace("Clock Speed: ", ""),
